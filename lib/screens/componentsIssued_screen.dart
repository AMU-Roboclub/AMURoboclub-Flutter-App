...
import 'package:roboclub_flutter_app/widgets/Components_card.dart';
...
class ComponentIssued extends StatefulWidget {
  const ComponentIssued({Key? key}) : super(key: key);

  @override
  _RegMembersScreenState createState() => _RegMembersScreenState();
}

class _RegMembersScreenState extends State<ComponentIssued> {
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();
  bool _isLoading = true;
  List<ComponentMember> membersList = [];
  ...
  List<ComponentMember> searchResult = [];

  bool showButton = false;
  late ComponentMember member;
  late int amount;
  File? pdf;
  @override
  void initState() {
    ...
    ComponentsIssuedService().fetchMembers().then((value) {
        addMemberList(value.cast<ComponentMember>());
        setState(() {
          _isLoading = false;
        });
      });
  ...
  Future<void> generateOrderId(ComponentMember mem) async {
    member = mem;
    // print(member);
  ...
  Future<void> generatePdf() async {
    ...
    // print(regNo);
    pdf = await PdfManager().createRegSlip(member as Member, regNo);
  }
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    if (digest.toString() == sig) {
      ...
      ComponentsIssuedService().updatePaymentStatus(member).then((value) async {
        await generatePdf();
      ...
      setState(() {
        member.isIssued = true;
      });
      ComponentsIssuedService().postPaymentDetails({
      ...
      showDialog(
          ...
          builder: (context) {
            return AlertDialog(
              content: Text(
                "Applied for Components Successful. You will recieve a mail with your registration slip.",
                style: kLabelStyle,
              ),
              actions: [
                TextButton(
  ...
  void addMemberList(List<ComponentMember> members) {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
        child: Scaffold(
            key: _scaffoldKey,
            drawer: appdrawer(context, page: "Issued Components"),
            appBar: appBar(
              context,
              strTitle: "Issued Components To Members",
              isDrawer: true,
